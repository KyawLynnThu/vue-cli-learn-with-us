...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <h2>
    <router-link class="navbar-brands ml-3 text-dark font-weight-bold" :to="{ name: 'Home' }" style="text-decoration: none">LearnWithUs</router-link>
  </h2>
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown mx-3">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
          <router-link :to="`/category/${cats.id}`" v-for="cats in cat" :key="cats.id" onClick="window.location.reload(true)" class="dropdown-item">{{ cats.name }}
          </router-link>
        </div>
      </li>
    </ul>
