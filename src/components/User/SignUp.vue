  ...
  <div class="col-lg-4 col-md-6 col-12 mx-auto">
    ...
    <form class="bg-light p-4">
      ...
      <div class="form-group">
        ...
        <input v-model="registerData.password_comfirm" type="password" class="form-control" required />
      </div>
      <div class="mb-3 mt-3">
        <router-link @click="register" :to="{ name: 'confirmSignup' }" type="submit" class="btn btn-dark btn-block text-uppercase font-weight-bold">
          Register
        </router-link>
      </div>
      <div>
        <router-link :to="{ name: 'LogIn' }" type="submit" class="
...
export default {
  ...
  methods: {
    register() {
      axios
        .post("http://127.0.0.1:8000/api/user/register", this.registerData)
        .then((res) => {
